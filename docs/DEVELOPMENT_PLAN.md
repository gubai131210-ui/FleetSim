...
| **11** | Lidar 感知层 + 多模态预测 + EECBS 教学版 + 交通灯 + 非线性 NMPC | 3–4 周 |
...
> Phase 11 完整 Goal 提示词：`docs/PHASE11_GOAL_PROMPT.md`（复制 §0 给 Goal Agent）。  
...
> Phase 11 验收：ASCII 外置 Build `D:\build\FleetSim_phase11*` + `FleetSimTests` 全绿 + `tools/verify_phase11_evidence.py`（会话 8 交付）；**12 项算法清单全部真实现**。
